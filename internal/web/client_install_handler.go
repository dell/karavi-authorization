...
kubectl get secrets,deployments,daemonsets -n vxflexos -o yaml \
  | karavictl inject --image-addr %s --proxy-host %s \
  | kubectl apply -f -
...
func ClientInstallHandler(imageAddr string) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		host := r.Host // TODO(ian): Potential risk of DNS rebinding attack.
		w.Write([]byte(fmt.Sprintf(InstallScriptFormat, imageAddr, host)))
	})
}
