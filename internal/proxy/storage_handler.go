...
func NewStorageHandler(log *logrus.Entry, client pb.StorageServiceClient) *StorageHandler {
	...
	mux.Handle(fmt.Sprintf("%s%s/", web.ProxyStoragePath, "create"), web.Adapt(web.HandlerWithError(sh.createHandler), web.TelemetryMW("csm-authorization-proxy-server", "storageCreateHandler", log)))
	...
	mux.Handle(fmt.Sprintf("%s%s/", web.ProxyStoragePath, "list"), web.Adapt(web.HandlerWithError(sh.deleteHandler), web.TelemetryMW("csm-authorization-proxy-server", "storageListHandler", log)))
