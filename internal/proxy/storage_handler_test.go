...
func TestStorageHandler(t *testing.T) {
	...
	t.Run("it handles storage getPowerflexVolumes", func(t *testing.T) {
		t.Run("successfully gets powerflex volumes", func(t *testing.T) {
			client := &mocks.FakeStorageServiceClient{
				GetPowerflexVolumesFn: func(ctx context.Context, ctr *pb.GetPowerflexVolumesRequest, co ...grpc.CallOption) (*pb.GetPowerflexVolumesResponse, error) {
			...
			r := httptest.NewRequest(http.MethodGet, "/proxy/storage/getPowerflexVolumes", nil)
			w := httptest.NewRecorder()

			q := r.URL.Query()
			q.Add("VolumeName", "volume1,volume2")
			q.Add("SystemId", "542a2d5f5122210f")
			r.URL.RawQuery = q.Encode()
		...
		t.Run("handles bad request", func(t *testing.T) {
			...
			r := httptest.NewRequest(http.MethodPost, "/proxy/storage/getPowerflexVolumes", nil)
			w := httptest.NewRecorder()
